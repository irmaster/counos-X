...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee)
{
    ...
    if (nSpendHeight > HeightOnlyTrustNodeCanMine) {
        burnedAddress = downloader.download("http://trust.counos.io/api/v1/ccx/addresses/burned?current_height=" + std::to_string(nSpendHeight));
    }
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        if (coin.IsCoinBase() && nSpendHeight - coin.nHeight < GetCOINBASE_MATURITY()) {
            return state.Invalid(false,
                REJECT_INVALID, "bad-txns-premature-spend-of-coinbase",
                strprintf("tried to spend coinbase at depth %d", nSpendHeight - coin.nHeight));
        }
        //Dont Allow to send coin from Burned Address
        if (nSpendHeight > HeightOnlyTrustNodeCanMine + 70000) {
